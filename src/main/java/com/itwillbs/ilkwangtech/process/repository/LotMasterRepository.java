...
public interface LotMasterRepository extends JpaRepository<LotMaster, String> {
	...
	@Query(value = "SELECT l.lot_id AS lotId, i.item_name AS itemName, l.status AS status, l.created_date AS createdDate " +
	            "FROM lot_master l " +
	...
	@Query(value = "SELECT l.lot_id AS lotId, i.item_name AS itemName, p.instruct_code AS instructCode, " +
	               "p.instruct_qty AS instructQty, p.status AS status, p.start_date AS startDate, " +
	               "p.end_date AS endDate, p.defective AS defective " +
	               "FROM lot_master l " +
