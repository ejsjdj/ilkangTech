...
function renderEmployeeTable(pageData) {
  ...
  if (!pageData || !pageData.content || pageData.content.length === 0) {
    html =
      '<div class="p-3 text-center text-muted">조회된 직원이 없습니다.</div>';
  } else {
...
function registAppointment() {
  ...
  if (isDeptChanged && isRankChanged) {
    ...
    generatedContent += `해당 인사의 업무 성과 달성 및 조직개편으로 인한 인사 발령(${currentDept} -> 생산관리팀, ${currentRank} -> 대리)이 있으니, 담당자분들은 확인하시고 승인/반려 해주시기 바랍니다.`;
    ...
    generatedContent += `인사평가 결과 목표 및 역량 달성으로 인한 인사 발령(${currentDept} -> 생산관리팀, ${currentRank} -> 대리)을 요청하오니, 담당자분들은 확인하시고 승인/반려 해주시기 바랍니다.`;
  } else {
